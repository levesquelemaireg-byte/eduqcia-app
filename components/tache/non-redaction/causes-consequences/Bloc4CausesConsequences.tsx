...
import {
  NR_CAUSES_CONSEQUENCES_BLOC4_INFO,
  NR_CAUSES_CONSEQUENCES_GATE_PRE_DOCS,
} from "@/lib/ui/ui-copy";

export function Bloc4CausesConsequences() {
  ...
  const np = normalizeCausesConsequencesPayload(nonRedactionCausesConsequencesPayload(state));
  const bloc3Ok = np !== null && isCausesConsequencesStep3Complete(np);

  if (!blueprintGate || !bloc3Ok) {
    return (
      <p className="text-sm leading-relaxed text-muted">{NR_CAUSES_CONSEQUENCES_GATE_PRE_DOCS}</p>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className="icon-lead rounded-xl border-l-4 border-accent bg-surface py-3.5 pl-4 pr-4 text-sm text-steel shadow-sm ring-1 ring-border/40">
        ...
        <span className="min-w-0 leading-relaxed">{NR_CAUSES_CONSEQUENCES_BLOC4_INFO}</span>
      </div>
