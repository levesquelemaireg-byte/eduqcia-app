...
type Props = {
  onClick: () => void;
  className?: string;
  /** Surcharge du libellé vocal (ex. distinguer deux aides sur le même champ). */
  ariaLabel?: string;
};

/** Bouton (i) — ouvre une modale d’aide ; gabarit aligné sur `LabelWithInfo` (`docs/DESIGN-SYSTEM.md`). */
export function FieldHelpModalButton({ onClick, className, ariaLabel }: Props) {
  return (
    <button
      type="button"
      onClick={(e) => {
      ...
      aria-label={ariaLabel ?? ARIA_OPEN_FIELD_HELP}
    >
