-- Suppression de la colonne print_impression_scale sur documents.
-- Le print-engine v2 gère l'échelle d'impression côté CSS/composants React,
-- cette colonne n'est plus utilisée.

-- 1. Supprimer la contrainte CHECK puis la colonne
ALTER TABLE documents DROP CONSTRAINT IF EXISTS documents_print_impression_scale_check;
ALTER TABLE documents DROP COLUMN IF EXISTS print_impression_scale;

-- 2. Recréer publish_tae_transaction sans print_impression_scale
...
-- 3. Recréer update_tae_transaction sans print_impression_scale
