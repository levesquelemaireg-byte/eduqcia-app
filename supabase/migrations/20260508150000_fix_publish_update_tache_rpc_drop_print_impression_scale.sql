-- Correctif : retirer la référence à `documents.print_impression_scale` dans
-- `publish_tache_transaction` et `update_tache_transaction`.
--
-- Contexte : la colonne `documents.print_impression_scale` a été supprimée
-- par la migration `20260416120000_drop_documents_print_impression_scale.sql`,
-- qui a recréé `publish_tae_transaction` et `update_tae_transaction` sans
-- cette colonne. Mais les deux migrations suivantes ont régressé en recréant
-- les RPC avec la colonne fautive :
--
--   - `20260422185919_rename_tae_to_tache.sql` (rename + recréation des RPC
--     sous le nouveau nom `*_tache_transaction`)
--   - `20260423110000_schema_cd1_rpc_persistence.sql` (ajout du champ
--     `schema_cd1_data` — re-recréation des RPC depuis l'état pré-drop)
--
-- En distant, l'application séquentielle de ces migrations laisse les RPC
-- avec un INSERT qui réfère une colonne inexistante. Toute publication ou
-- mise à jour de tâche échoue avec :
--
--   « column "print_impression_scale" of relation "documents" does not exist »
--
-- Cette migration recrée les deux RPC sans la colonne. Idempotente
-- (CREATE OR REPLACE), pas de migration de données.
--
-- Référence : `docs/specs/print-engine.md` v2 — l'échelle d'impression est
-- gérée côté CSS/composants React et n'a plus de représentation persistée.

-- ============================================================
-- publish_tache_transaction (création initiale d'une TAÉ + documents)
-- ============================================================
...
-- ============================================================
-- update_tache_transaction (mise à jour d'une TAÉ existante)
-- ============================================================
