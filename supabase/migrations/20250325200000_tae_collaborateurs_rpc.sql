-- TAE collaborateurs: apply_tae_collaborateurs_from_payload + publish_tae_transaction + update_tae_transaction
-- Source: supabase/schema.sql (TAE collaborateurs RPC)
...
-- ============================================================
-- RPC : publication TA� (transaction atomique)
-- Appel�e depuis Next.js : supabase.rpc('publish_tae_transaction', { p_payload: <objet> })
...
RETURNS uuid
LANGUAGE plpgsql
-- SECURITY DEFINER + d�sactivation RLS dans le corps : �vite la r�cursion infinie entre
-- tae_docs_write ? auth_can_edit_tae() ? SELECT sur tae ? tae_select / collaborateurs.
-- L�auteur est forc� � auth.uid() avant toute �criture.
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
...
BEGIN
  ...
  -- R�serv� au r�le propri�taire de la fonction (postgres) : contourne RLS pour cette transaction locale uniquement.
  ...
  IF (p_payload->'tae'->>'conception_mode') = 'equipe' THEN
    IF NOT EXISTS (SELECT 1 FROM tae_collaborateurs WHERE tae_id = v_tae_id) THEN
      RAISE EXCEPTION 'publish_tae_transaction: collaborateur actif requis (mode equipe)';
    END IF;
  END IF;

  -- Brouillon wizard : si la table n�est pas encore migr�e sur le projet Supabase, ne pas faire �chouer la publication.
  IF EXISTS (
    SELECT 1
...
-- ============================================================
-- RPC : mise � jour TA� (m�me forme de payload que publish_tae_transaction)
-- ============================================================
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM evaluation_tae WHERE tae_id = p_tae_id) THEN
    RAISE EXCEPTION 'update_tae_transaction: tae utilis�e dans une �valuation';
  END IF;
  ...
  IF (p_payload->'tae'->>'conception_mode') = 'equipe' THEN
    IF NOT EXISTS (SELECT 1 FROM tae_collaborateurs WHERE tae_id = p_tae_id) THEN
      RAISE EXCEPTION 'update_tae_transaction: collaborateur actif requis (mode equipe)';
    END IF;
  END IF;
