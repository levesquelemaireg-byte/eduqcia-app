...
Le code existant (`lib/tache/`, `TaeCard`, `TaeFicheData`, tables SQL `tae`, `tae_collaborateurs`, RPCs `publish_tae_transaction`, etc.) est **grandfathered** : il n'est pas renommé sauf migration progressive planifiée (voir backlog technique dans [BACKLOG.md](./BACKLOG.md#backlog-technique-détaillé)).
