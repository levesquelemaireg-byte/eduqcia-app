...
Tout **nouveau** code (types, fichiers, dossiers, variables) doit utiliser `tache` / `Tache` (français). Le préfixe `tae` / `Tache` est **interdit dans les nouveaux fichiers** — il reste un héritage technique de la phase alpha.

Le code existant (`lib/tache/`, `TacheCard`, `TacheFicheData`, tables SQL `tae`, `tae_collaborateurs`, RPCs `publish_tae_transaction`, etc.) est **grandfathered** : il n'est pas renommé sauf migration progressive planifiée (voir backlog technique dans [BACKLOG.md](./BACKLOG.md#backlog-technique-détaillé)).
...
- **`@page`** : format **Letter portrait**, **marges 2 cm** sur chaque côté — `PrintPreviewModal` / `lib/tache/print-page-css.ts` (`TACHE_PRINT_PAGE_MARGIN`, aligné sur **`--tache-print-sheet-padding`**, retirées à la fermeture de la modale). En **`@media print`**, **`#tache-wizard-printable-fiche`** : **`padding: 0`**, **`width: 100%`** de la zone utile (`app/globals.css`). **Aperçu écran** : feuille **`.paper`** avec **`padding` 2 cm** via **`--tache-print-sheet-padding`**. Masquer tout l’**`AppShell`** à l’impression pour ne laisser que la feuille.
- **En-têtes et pieds du navigateur** (date, URL, pagination) : réglage de la boîte d’impression ; texte d’aide utilisateur dans [UI-COPY.md](./UI-COPY.md). Route **`/questions/[id]/print`** : même logique (`TachePrintRouteShell`, `TacheFichePrintView`).
...
- **Réglage échelle à l’impression :** variable CSS **`--tache-doc-print-figure-scale`** et `calc()` sur **`--tache-print-document-figure-max-height`** appliqué au **`max-height`** de l’`<img>` (fiche / aperçu) ; éviter `transform: scale()` comme effet principal pour la netteté (légendes, PDF).
