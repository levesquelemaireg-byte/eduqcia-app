...
**Fiche et impression :** l’**enseignant** voit corrigé et options selon le payload non rédactionnel ; l’**élève** voit le gabarit prévu (grille, segments, etc.) sans exposer le corrigé enseignant — aligné **`FicheTache`**, **`formStateToTae`**, bundles impression.
...
| Guidage élève (`tae.guidage`)       | **Texte fixe** `NR_ORDRE_STUDENT_GUIDAGE` — non modifiable par l’enseignant ; distinct de l’aide **options** (`NR_ORDRE_OPTIONS_HELP`). Sur **feuille élève** (aperçu impression, questionnaire épreuve), masquable via `TaeFicheData.showGuidageOnStudentSheet === false` (ex. passation **sommative**). |
...
**Implémentation (app) :** slug **`avant-apres`** — `Bloc3AvantApres`, `Bloc4AvantApres`, `Bloc5AvantApres` ; état wizard `bloc5.nonRedaction` = `{ type: 'avant-apres'; payload: AvantApresPayload }` ; persistance **`tae.non_redaction_data`** (JSONB, même union à la publication) via `publish_tae_transaction` / `update_tae_transaction` ; builders HTML `avant-apres-payload.ts` + `publish-tae-payload.ts` ; réhydratation édition `parseNonRedactionData`, `non-redaction-edit-hydrate.ts`, `load-tae-for-edit.ts` ; **repère temporel obligatoire** sur les quatre documents (`lib/tae/behaviours/avant-apres.ts`) ; registre copy **`NR_AVANT_APRES_*`**, toast **`TOAST_TAE_NR_AVANT_APRES_HYDRATE_INVALID`** ; impression / épreuve : `AvantApresPrintableQuestionnaireCore`. Les parcours **1.1** / **1.2** restent sans JSON colonne (NULL) jusqu’à harmonisation — voir [BACKLOG.md](./BACKLOG.md).
