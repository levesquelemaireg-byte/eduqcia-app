...
| Tâche                                                     | **Tâche** (forme officielle ; ancienne forme longue « tâche d'apprentissage et d'évaluation » et acronyme TAÉ interdits en UI depuis le 25 avril 2026) | `DonneesTache` / `Tache` (**jamais `Task`**)                                                                    | `tache` (depuis Phase 0, 22 avril 2026 — renommé depuis `tae`) | `lib/tache/contrats/donnees.ts`                        |
...
| Consigne                                                  | **Consigne**                                                                                                                                           | `FormState.redaction.consigne`                                                                                  | `tache.consigne`                                               | `lib/tache/tache-form-state-types.ts`                  |
| Guidage complémentaire                                    | **Guidage complémentaire**                                                                                                                             | `Guidage` / `FormState.redaction.guidage`                                                                       | `tache.guidage`                                                | `lib/tache/contrats/donnees.ts:16`                     |
| Production attendue                                       | **Production attendue** (UI) / `corrige` (code/DB) — **divergence assumée**                                                                            | `FormState.bloc5.corrige` / `selectCorrige`                                                                     | `tache.corrige`                                                | `lib/fiche/selectors/selectCorrige.ts`                 |
| Notes au correcteur                                       | **Notes au correcteur**                                                                                                                                | `FormState.bloc5.notesCorrecteur`                                                                               | **colonne SQL à créer** — `tache.notes_correcteur`             | `lib/tache/tache-form-state-types.ts`                  |
...
| --- | -------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| D1  | Tâche                            | `tache` (SQL depuis 22 avril 2026), `Tache*` (composants), `/questions` (route) | **SQL + composants renommés** (Phase 0). Reste la route `/questions` — renommage non planifié (impact URL publique) | Accepté : SQL et code alignés sur `tache`. Route `/questions` conservée (convention existante). |
...
| D10 | Notes au correcteur              | `notesCorrecteur` en FormState, **aucune colonne SQL**                          | Colonne manquante                                                                                                   | Créer `tache.notes_correcteur` en DB                                                            |
...
**État wizard** : `FormState` à 7 blocs (`lib/tache/tache-form-state-types.ts`).

**Attributs principaux** :

- Consigne (`tache.consigne`)
- Appel documentaire (dérivé, voir §3.3)
- Guidage complémentaire (optionnel, `tache.guidage`)
- Références ordonnées à des documents (0 à N, fixé par le comportement attendu, table `tache_documents`)
...
- Production attendue / clé de correction (`tache.corrige`)
...
- Pied de page
- Indexation propre
- Sélection ordonnée de tâches (table pivot `evaluation_tache`)
...
**En code** : `components/tache/wizard/`, stepper dans `Stepper.tsx`, méta-étapes dans `step-meta.ts` (`TACHE_FORM_STEPS`).
...
**Table SQL actuelle** : `favoris` avec enum `favori_type = ('tache','document','evaluation')`. **Dette D8** : renommer `favoris` → `ressources_epinglees`, et aligner les valeurs de l'enum avec les noms canoniques.
...
2. **Un document créé in situ est autonome dès la publication de la tâche.** La création du document est couplée à la publication de la tâche via `publish_tache_transaction` pour éviter les documents orphelins. Tant que la tâche n'est pas publiée, les données du document ne vivent que dans le brouillon du wizard.
...
> Vérifie INV-OI1 à INV-OI3 et INV-N1 à INV-N4. Identifie : les OI sans rendus complets, les occurrences `Tache` dans le code, les occurrences `OI` dans les UI, les `evaluation/evaluations` à renommer, les `amorce` à renommer en `appel`.
