...
components/
  ...
  tache/           # TacheForm, impression tâche, grilles, variantes non rédactionnelles
  prototype/
lib/
  ...
  tache/           # helpers métier, publish, hydrate, variantes non rédactionnelles, impression tâche
  ...
  types/, schemas/
  images/          # Pipeline upload images — `compress-uploaded-image.ts` conserve la taille originale, compresse en JPEG dégressif uniquement si > 2 Mo (**sharp**) ; consommé par `lib/actions/documents.ts` (`uploadTacheDocumentImageAction`)
...
| Co-conception              | `tache_collaborateurs` — le collaborateur peut modifier, pas supprimer.                                                                                                                     |
...
| `tache.non_redaction_data` | JSONB nullable — parcours NR structurés (ex. **1.3** `{ type, payload }`) ; 1.1 / 1.2 **NULL** tant que non migrés ; RPC `CASE WHEN p_payload->'tache' ? 'non_redaction_data'` en `UPDATE`. |
...
| Brouillon wizard           | `tache_wizard_drafts` — une ligne par `user_id`, `payload` JSONB.                                                                                                                           |

### RPC `publish_tache_transaction`

- **Rôle :** publier une TAÉ en une transaction : documents nouveaux, ligne `tache`, `tache_documents`, suppression du brouillon `tache_wizard_drafts` pour l’utilisateur courant (si la table existe).
- **Signature :** `publish_tache_transaction(p_payload jsonb) RETURNS uuid`.
...
- **Appel :** `supabase.rpc('publish_tache_transaction', { p_payload: <objet> })` — objet JS, pas une chaîne seule.
...
- **Payload :** `auteur_id`, `tache` (colonnes publiables, dont **`non_redaction_data`** JSONB optionnel pour le parcours **1.3**), `documents_new[]` (entrées avec `elements` JSONB, métadonnées pédagogiques), `slots` (`mode` reuse/create, `document_id`, `newIndex`, etc.), `collaborateurs_user_ids[]` (UUID `profiles.id` actifs, exclus l’auteur — requis si `tache.conception_mode` = `equipe`). Insertion via `apply_tache_collaborateurs_from_payload` dans `supabase/schema.sql`.
- **Définition complète et `GRANT` :** fin de `supabase/schema.sql`.

### RPC `update_tache_transaction`

- **Rôle :** mettre à jour une TAÉ existante (wizard **Modifier**) en une transaction : nouveaux documents éventuels, `UPDATE tache`, remplacement des lignes `tache_documents`. Refus si la TAÉ figure dans `evaluation_tache`.
- **Signature :** `update_tache_transaction(p_tae_id uuid, p_payload jsonb) RETURNS uuid`.
- **`non_redaction_data` :** si la clé est **absente** sous `p_payload->'tache'`, la colonne existante est **conservée** ; si la clé est **présente** (y compris `null`), la valeur est **remplacée** — voir `CASE WHEN … ? 'non_redaction_data'` dans `supabase/schema.sql`.
- **Appel côté app :** `lib/tache/publish-tache.ts` (`updateTacheFromFormState`) — `supabase.rpc('update_tache_transaction', { p_tae_id, p_payload })`.
...
- **Rôle :** créer ou mettre à jour une **épreuve** (`evaluations`, titre, ordre des TAÉ dans `evaluation_tache`), en brouillon ou en publiant (`p_publish = true` impose au moins une TAÉ).
- **Signature :** `save_evaluation_composition(p_evaluation_id uuid, p_titre text, p_tae_ids uuid[], p_publish boolean) RETURNS uuid` — `p_evaluation_id` **NULL** = insert `evaluations` + lignes `evaluation_tache`.
...
- **Sécurité :** `SECURITY DEFINER`, `search_path = public`, compte actif, `auteur_id` vérifié sur mise à jour ; `set_config('row_security', 'off', true)` pour les opérations sur `evaluation_tache`.
...
- **Rôle :** suppression de compte Loi 25 — anonymise le profil (`first_name` = '[Compte', `last_name` = 'supprimé]'`, email nul, school_id nul, status = suspended), vide les tables pivot (`profile_disciplines`, `profile_niveaux`), supprime les brouillons non publiés (`tache`+`documents`+`evaluations`WHERE`is_published = false`).
...
| `tache`                                                                | Tâche — pas de colonne `titre`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `tache_collaborateurs`, `tache_wizard_drafts`, `tache_versions`        | Co-conception, brouillon, archives version majeure.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `documents`, `tache_documents`                                         | Documents historiques, slots **`doc_1`..`doc_N`** (contrainte `CHECK (slot ~ '^doc_[1-9][0-9]?$')` depuis Phase 1, 22 avril 2026 ; auparavant `doc_A..D`). Colonnes **`repere_temporel`**, **`annee_normalisee`** (repère temps enseignant, année pour algorithmes OI1 non rédactionnels). **`type_iconographique`** (TEXT, optionnel) : sous-catégorie didactique si **`type` = iconographique** — filtre banque, affichage fiche / liste ; validation applicative. **`is_published`** sur `documents` : visibilité **banque** — `false` pour les documents **créés lors de la publication d’une TAÉ** jusqu’à complétion (fiche document) ; `true` pour création autonome. Colonnes **`source_type`**, **`image_legende`**, **`image_legende_position`** — voir [FEATURES.md](./FEATURES.md) §5–5.6. |
| `votes`, `votes_archives`, `tache_usages`                              | Votes, archive, usage (déverrouillage vote).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `commentaires`                                                         | Commentaires sur TAÉ publiées.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `evaluations`, `evaluation_tache`                                      | **Épreuve** en copy UI — tables SQL `evaluations` et liaisons `evaluation_tache` (ordre des tâches dans l’épreuve).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
**RPC sur environnements existants :** les projets Supabase provisionnés **avant** l’ajout de `update_tache_transaction` dans `schema.sql` doivent appliquer la migration `supabase/migrations/20250325180000_update_tae_transaction.sql` (SQL Editor ou `supabase db push` après `supabase link`). Sinon l’édition renvoie PGRST202 ; message utilisateur et copy : [DECISIONS.md](./DECISIONS.md) (toasts Créer une TAÉ).

### Checklist RLS (manuelle)

Avant démo ou go-live, exécuter la procédure **[RLS-CHECKLIST.md](./RLS-CHECKLIST.md)** sur **staging ou prod** (ou local + projet Supabase réel) avec **deux comptes** enseignants actifs. La checklist est **par vagues** : **1–2** (brouillons) ; **3–4** (épreuves brouillon + composition) — **OK documenté** sur env local (27 mars 2026, voir fiche de passage dans la checklist) ; rejouer sur **staging/prod** avant go-live si besoin ; **5–7** surtout **après** la **banque collaborative** — voir [BACKLOG.md](./BACKLOG.md) §3 et [RLS-CHECKLIST.md](./RLS-CHECKLIST.md). Politiques dans `supabase/schema.sql` (`tache_select`, `tache_wizard_drafts_*_own`, `eval_tache_*`, `favoris_own`, etc.).
...
- **Impression fiche TAÉ :** feuille **Letter** — variables **`--tache-print-sheet-*`** sur `:root` (`app/globals.css`) : largeur **8,5 in**, hauteur min. **11 in**, **`--tache-print-sheet-padding` 2 cm** sur **`.paper`** (aperçu écran). **Impression Puppeteer (route SSR `/apercu/[token]`) :** **`@page { margin: 2cm; size: letter portrait }`** dans `styles/impression.css`. Corps fiche (hors barèmes) : **Arial**, **11pt** / **line-height 1.5** (`styles/impression.css` + `.paper`) ; les grilles gardent **Arial** et leurs tailles sur **`table`** (`eval-grid.module.css`, grilles dédiées). Section **documents** : grille `docsGrid` (`auto-fit` / `minmax`, hybride `:has` + `docsGridHasIcono`) ; **document iconographique** : **`.documentCell`** **`max-content`** + **`justify-self: start`** ; figures avec bornes **`PRINT_IMAGE_MAX_WIDTH_PX = 660`** et **`PRINT_IMAGE_MAX_HEIGHT_PX = 350`** (`lib/impression/constantes-image.ts`) — exposées en CSS via **`max-width: min(100%, 660px)`** et **`--tache-print-document-figure-max-height = 9.3cm`** sur **`.documentFigureImg`**, **`.documentFigure`** **`inline-block`** / **`max-width: 100%`** (`components/document/renderer/printable-fiche-preview.module.css`).
