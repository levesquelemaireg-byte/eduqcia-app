...
Un environnement DEV pour comparer **le même jeu de données fiche** (`TacheFicheData`, `lib/types/fiche.ts`) sous **plusieurs contextes d'affichage** réels du produit, et ce **par comportement attendu** (`comportement_id` dans `public/data/oi.json`).
...
| Thumbnail | `TacheCard`                                                                                             | Carte condensée (liste / banque)       |
| Print     | `PrintableFicheFromTacheData` — **pas** `PrintableFichePreview` (celui-ci dépend du `FormState` wizard) | Deux feuillets dossier / questionnaire |

**Mocks :** `lib/fragment-playground/mocks.ts` — une entrée `MOCK_TACHE_FICHE_BY_COMPORTEMENT_ID` par comportement **sélectionnable** avec mock (15 ids : 0.1, 1.1–1.3, 3.1–3.5, 4.1–4.2, 6.1–6.3, 7.1). `outilEvaluation` aligné sur `oi.json`. Parcours **1.1, 1.2, 1.3** : HTML consigne / corrigé / guidage produit par les **mêmes builders** que la publication (`buildOrdreChronologiqueConsigneHtml`, `buildLigneDuTempsConsigneHtml`, `runAvantApresGeneration` + `buildAvantApresConsigneHtml`, etc.) pour satisfaire les parseurs impression.
...
- **Vue complète** — composant produit tel quel (`FicheTache`, `TacheCard`, `PrintableFicheFromTacheData`) lorsque le mode **Debug** est désactivé.
...
**Print — vue complète + debug :** un seul enveloppe `data-fragment="PrintableFicheFromTacheData"` autour du composant prod (le contenu interne des deux feuillets n’est pas découpé sans dupliquer tout le module d’impression). Utiliser **Fragment isolé** (`PrintDossier`, `PrintQuestionnaire`, etc.) pour des repères par bloc.
...
lib/fragment-playground/
  ...
  mocks.ts                      → `MOCK_TACHE_FICHE_BY_COMPORTEMENT_ID`, `getMockTacheFicheForComportement`
...
components/playground/
  ...
  PlaygroundTacheCardRenderer.tsx   → `TacheCard` ou sous-blocs (isolé / debug)
  PlaygroundPrintRenderer.tsx     → `PrintableFicheFromTacheData` ou blocs print isolés
...
2. Ajouter une entrée dans `MOCK_TACHE_FICHE_BY_COMPORTEMENT_ID` typée **`TacheFicheData`** (`lib/types/fiche.ts` uniquement — ne pas recréer le type ailleurs).
...
Marges   : 2 cm partout  (TACHE_PRINT_PAGE_MARGIN, --tache-print-sheet-padding)
...
```
✅ lib/types/fiche.ts              → type canonique `TacheFicheData` (jamais dupliqué dans le playground)
...
| Besoin                             | Fichier                                                                |
| ---------------------------------- | ---------------------------------------------------------------------- |
| Données fiche                      | `lib/types/fiche.ts` (`TacheFicheData`)                                |
...
| Impression depuis `TacheFicheData` | `PrintableFicheFromTacheData` dans `PrintableFichePreview.tsx`         |
