...
- **Guidage publié (`tache.guidage`) — élève :** **`NR_AVANT_APRES_ELEVE_GUIDAGE`**.
- **Feuille élève (impression) :** intro publiée **`NR_AVANT_APRES_PUBLISHED_INTRO_*`** ; tableau des options — en-têtes **`NR_AVANT_APRES_ELEVE_SHEET_TABLE_COL_AVANT`** / **`NR_AVANT_APRES_ELEVE_SHEET_TABLE_COL_APRES`**, **`NR_AVANT_APRES_ELEVE_SHEET_TABLE_REPERE_TH_SR`** (colonne pivot sans titre visible) ; **`NR_AVANT_APRES_ELEVE_SHEET_*`** ; styles **`[data-avant-apres-eleve="true"]`** (`app/globals.css`). **Wizard Bloc 5 :** en-têtes explicites **`NR_AVANT_APRES_TABLE_COL_*`** (distincts de la feuille élève).
...
**Registre :** constantes **`NR_CARTE_*`** et toast **`TOAST_TACHE_NR_CARTE_HISTORIQUE_HYDRATE_INVALID`** dans `lib/ui/ui-copy.ts` (spec archivée [specs/fermees/spec-oi2-carte-historique.md](./specs/fermees/spec-oi2-carte-historique.md)). **Branchement :** slug `carte-historique` — `TACHE_NON_REDACTION_WIZARD_BLOCS`, `Bloc3CarteHistorique`, `Bloc4CarteHistorique`, `Bloc5CarteHistorique`, action `NON_REDACTION_PATCH_CARTE_HISTORIQUE`, `carte-historique-payload.ts` / `carte-historique-helpers.ts`, colonne **`tache.non_redaction_data`**.
...
**Registre :** constantes **`NR_MANIFESTATIONS_*`** dans `lib/ui/ui-copy.ts` (spec archivée [specs/fermees/spec-oi5-mettre-relation-faits.md](./specs/fermees/spec-oi5-mettre-relation-faits.md)). **Branchement :** slug `manifestations` — `TACHE_NON_REDACTION_WIZARD_BLOCS`, `Bloc3Manifestations`, `Bloc4Manifestations`, `Bloc5Manifestations`, action `NON_REDACTION_PATCH_MANIFESTATIONS`, `manifestations-payload.ts` / `manifestations-helpers.ts`, colonne **`tache.non_redaction_data`**.

- **Bloc 2 — Organisation des catégories (5.2 uniquement) :** `OrganisationCategoriesPicker` — **`BLOC2_ORGANISATION_CATEGORIES_LABEL`**, **`BLOC2_ORGANISATION_CATEGORIES_HELP`**, options **`BLOC2_ORGANISATION_CATEGORIES_OPTION_2`** / **`BLOC2_ORGANISATION_CATEGORIES_OPTION_4`** ; récap dans `BlueprintLockedView` — **`BLOC2_ORGANISATION_CATEGORIES_BLUEPRINT_LABEL`**.
- **Étape 3 — consigne et catégories :** **`ManifestationsConsigneTemplate`** (réutilise `ConsigneTemplateCard` + `LimitCounterPill`) — `EditableInput` pour le sujet général, `DocToken` adapté **`NR_MANIFESTATIONS_51_DOC_TOKEN`** (« {{doc_1}} et {{doc_2}} ») ou **`NR_MANIFESTATIONS_52_DOC_TOKEN`** (« {{doc_1}} à {{doc_4}} »), instructions **`NR_MANIFESTATIONS_51_INSCRIVEZ`** / **`NR_MANIFESTATIONS_52_INSCRIVEZ`**, légendes communes du template carte-historique. Aides **`NR_MANIFESTATIONS_CONSIGNE_HELP_51`** / **`NR_MANIFESTATIONS_CONSIGNE_HELP_52`** ; modale **`NR_MANIFESTATIONS_CONSIGNE_INFO_MODAL_BODY`** ; placeholders sujet **`NR_MANIFESTATIONS_SUJET_PLACEHOLDER_51`** / **`NR_MANIFESTATIONS_SUJET_PLACEHOLDER_52`** ; longueurs `MANIFESTATIONS_LIMITS.consigneSujet` (5.1 : 150/120, 5.2 : 200/160). **Catégories :** N champs avec `LimitCounterPill` 60/50 — **`NR_MANIFESTATIONS_CATEGORIES_SECTION_TITLE`**, **`NR_MANIFESTATIONS_CATEGORIES_SECTION_HINT`**, **`NR_MANIFESTATIONS_CATEGORIE_LABEL_PREFIX`** + index, placeholder **`NR_MANIFESTATIONS_CATEGORIE_PLACEHOLDER`**. **Guidage complémentaire :** TipTap libre, modale **`NR_MANIFESTATIONS_GUIDAGE_INFO_MODAL_BODY`**.
- **Étape 4 — documents :** **`Bloc4Manifestations`** réutilise `DocumentSlotsAccordion*` ; bannière info **`NR_MANIFESTATIONS_BLOC4_INFO_51`** / **`NR_MANIFESTATIONS_BLOC4_INFO_52`** ; gate **`NR_MANIFESTATIONS_GATE_PRE_DOCS`**.
- **Étape 5 — corrigé (assignation docs → catégories) :** **`Bloc5Manifestations`** — **`NR_MANIFESTATIONS_BLOC5_TITLE`**, **`NR_MANIFESTATIONS_BLOC5_HELP`** ; un ou deux `ListboxField` par catégorie selon organisation, options déjà assignées `disabled` ; placeholder **`NR_MANIFESTATIONS_BLOC5_DOC_PLACEHOLDER`**, label **`NR_MANIFESTATIONS_BLOC5_DOC_LABEL_PREFIX`** + index, aria **`NR_MANIFESTATIONS_BLOC5_DOC_PICKER_ARIA_PREFIX`** + index ; gate **`NR_MANIFESTATIONS_GATE_BLOC5`**.
- **Feuille élève (publication) :** `tache.consigne` = intro publiée + grille de cellules (label + 1 ou 2 cases avec séparateur **`NR_MANIFESTATIONS_ET_SEPARATOR`** pour 5.2-2cat) — **`NR_MANIFESTATIONS_PUBLISHED_INTRO_PREFIX`** / **`NR_MANIFESTATIONS_PUBLISHED_INTRO_SUFFIX`** ; styles `[data-manifestations-eleve="true"]` (`app/globals.css`).
- **Corrigé (`tache.corrige`) :** **`NR_MANIFESTATIONS_CORRIGE_TITRE`** + liste `<catégorie> : <num>` (ou `<num> et <num>` pour 5.2-2cat avec **`NR_MANIFESTATIONS_CORRIGE_TEMPLATE`**).

### Parcours non rédactionnel — causes et conséquences (OI4 · comportements 4.3, 4.4)

**Registre :** constantes **`NR_CAUSES_CONSEQUENCES_*`** dans `lib/ui/ui-copy.ts` (spec archivée [specs/fermees/spec-oi-4-3-4-4.md](./specs/fermees/spec-oi-4-3-4-4.md)). **Branchement :** slug `causes-consequences` — `TACHE_NON_REDACTION_WIZARD_BLOCS`, `Bloc3CausesConsequences`, `Bloc4CausesConsequences`, `Bloc5CausesConsequences`, action `NON_REDACTION_PATCH_CAUSES_CONSEQUENCES`, `causes-consequences-payload.ts`, colonne **`tache.non_redaction_data`**.

- **Mécanique invariante :** **toujours 2 documents et 2 catégories** ; les catégories sont **fixes** dérivées du comportement (4.3 → « Un facteur explicatif de {sujet} » × 2 ; 4.4 → « Une cause de {sujet} » + « Une conséquence de {sujet} ») via `getCausesConsequencesCategoryLabels`. Pas de saisie de catégorie par l'enseignant. **Première OI hybride** : 4.1 / 4.2 restent rédactionnels, 4.3 / 4.4 non rédactionnels — discrimination par `variant_slug`.
- **Étape 3 — consigne :** **`CausesConsequencesConsigneTemplate`** à 2 layouts. **4.3** : intro mono-paragraphe avec `EditableInput` pour le sujet — **`NR_CAUSES_CONSEQUENCES_43_CONSIGNE_PREFIX`** + sujet + **`NR_CAUSES_CONSEQUENCES_43_CONSIGNE_SUFFIX`**. **4.4** : intro **`NR_CAUSES_CONSEQUENCES_44_CONSIGNE_INTRO`** + liste à puces — première puce **`NR_CAUSES_CONSEQUENCES_44_BULLET_CAUSE_PREFIX`** + sujet (saisi UNE fois) ; deuxième puce **`NR_CAUSES_CONSEQUENCES_44_BULLET_CONSEQUENCE_PREFIX`** + reflet lecture seule via `EditableMirror`. Aides **`NR_CAUSES_CONSEQUENCES_CONSIGNE_HELP_43`** / **`NR_CAUSES_CONSEQUENCES_CONSIGNE_HELP_44`** ; modale **`NR_CAUSES_CONSEQUENCES_CONSIGNE_INFO_MODAL_BODY`** ; placeholders sujet **`NR_CAUSES_CONSEQUENCES_SUJET_PLACEHOLDER_43`** / **`NR_CAUSES_CONSEQUENCES_SUJET_PLACEHOLDER_44`** ; longueur `CAUSES_CONSEQUENCES_LIMITS.consigneSujet` (150/120). **Guidage complémentaire :** TipTap libre, modale **`NR_CAUSES_CONSEQUENCES_GUIDAGE_INFO_MODAL_BODY`**.
- **Étape 4 — documents (toujours 2) :** **`Bloc4CausesConsequences`** réutilise `DocumentSlotsAccordion*` ; bannière info **`NR_CAUSES_CONSEQUENCES_BLOC4_INFO`** ; gate **`NR_CAUSES_CONSEQUENCES_GATE_PRE_DOCS`**.
- **Étape 5 — corrigé :** **`Bloc5CausesConsequences`** — **`NR_CAUSES_CONSEQUENCES_BLOC5_TITLE`**, **`NR_CAUSES_CONSEQUENCES_BLOC5_HELP`** ; 2 `ListboxField` (un par rôle causal), labels dérivés via `getCausesConsequencesCategoryLabels`, l'option assignée à l'autre rôle est `disabled` (validation no-doublon UI) ; placeholder **`NR_CAUSES_CONSEQUENCES_BLOC5_DOC_PLACEHOLDER`**, aria **`NR_CAUSES_CONSEQUENCES_BLOC5_DOC_PICKER_ARIA_PREFIX`** + index ; gate **`NR_CAUSES_CONSEQUENCES_GATE_BLOC5`**.
- **Feuille élève (publication) :** `tache.consigne` = intro (4.3 mono / 4.4 liste à puces) + grille empilée 1 colonne (label + case) ; labels dérivés via préfixes **`NR_CAUSES_CONSEQUENCES_43_LABEL_PREFIX`** / **`NR_CAUSES_CONSEQUENCES_44_CAUSE_LABEL_PREFIX`** / **`NR_CAUSES_CONSEQUENCES_44_CONSEQUENCE_LABEL_PREFIX`** ; styles `[data-causes-consequences-eleve="true"]` (`app/globals.css`).
- **Corrigé (`tache.corrige`) :** **`NR_CAUSES_CONSEQUENCES_CORRIGE_TITRE`** + liste `<label dérivé> : <num>` (toujours 2 items distincts).
