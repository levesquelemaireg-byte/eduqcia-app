...
export function nonRedactionOrdrePayload(state: TacheFormState): unknown {
...
export function nonRedactionLignePayload(state: TacheFormState): unknown {
  const nr = state.bloc5.nonRedaction;
  if (nr?.type !== "ligne-du-temps") return null;
  return nr.payload;
}

export function nonRedactionAvantApresPayload(state: TacheFormState): unknown {
  const nr = state.bloc5.nonRedaction;
