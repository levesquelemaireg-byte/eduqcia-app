...
export async function getMyTaeThumbnailList(
  filtre: MyTaeListFiltre = "toutes",
): Promise<MyTaeThumbnailRow[]> {
  ...
  const taeRows: MyTaeThumbnailRow[] = rows.map((r) => {
  ...
  const wizardRow: MyTaeThumbnailRow = {
