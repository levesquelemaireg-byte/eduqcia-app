/**
 * Regénère `public/data/import-tache-notebooklm-bundle.json` en embarquant une copie
 ...
 *
 * Usage : `node scripts/build-import-tache-notebooklm-bundle.mjs`
 */
...
const outPath = join(root, "public", "data", "import-tache-notebooklm-bundle.json");
...
const bundle = {
  ...
  bundle_meta: {
    ...
    public_data_referentiels_policy:
      "Référentiels public/data/ immuables sauf nécessité absolue — docs/DECISIONS.md. Régénération : node scripts/build-import-tache-notebooklm-bundle.mjs",
  ...
  normalisation_cote_app_fr: {
    ...
    description:
      "Après parsing JSON, l'importateur peut appeler normalizeDocumentsNewTypesFromLlm sur documents_new pour corriger les alias anglais les plus fréquents avant validateTacheImportVsOi et la RPC. Cela complète le bundle ; ce n'est pas documenté comme valeur alternative officielle dans ce fichier — les gabarits restent 100 % français.",
  },
  ...
  post_import_comportement_cible_app: {
    resume:
      "Parser → normaliser alias LLM sur documents_new[].type (normalize-llm-aliases) → retirer connaissances_hors_rpc après résolution → connaissances_ids → validateTacheImportVsOi → injecter auteur_id → synthèse lacunes (images, etc.) → complétion → RPC.",
