...
## Audit sécurité et robustesse — clôturé 2026-04-17

Audit en 9 lots. Résumé des actions livrées :

| Lot | Scope                                                                     | Commit    |
| --- | ------------------------------------------------------------------------- | --------- |
| 1   | Tests ciblés open-redirect, hydration, guards                             | `7e23905` |
| 2   | Rate limiting, count estimated, CSP sans unsafe-eval                      | `79fd1bb` |
| 3   | Deps vulnérables, CI audit, sanitisation collaborateurs, proxy            | `ee98db3` |
| 4   | Deps vulnérables, CI audit, sanitisation collaborateurs, proxy            | `ee98db3` |
| 5   | Tests ciblés, fix open redirect regex, fix hydration                      | `7e23905` |
| 6   | Carrousel : clés stables, position onglets, copy invalidation             | `60a7bac` |
| 7   | Promise.all tokens épreuve, service_role → auth client dashboard          | `c45d6d2` |
| 8   | Découpage fichiers volumineux (ui-copy → 8 modules, reducer perspectives) | `8d61898` |
| 9   | Cache référentiels (unstable_cache 1h), doc print engine + observabilité  | `769be01` |

### Décisions explicites (pas d'action requise)

- **Email dans profiles** : conservé tel quel (décision développeur).
- **Filtres provisoires banque** : pas de correction, refonte totale planifiée.

### Limitations documentées (non corrigées, plan en place)

- **Pagination print engine** : mesureurPlaceholder 200px fixe + overflow hidden. Plan 3 phases documenté dans `lib/epreuve/PRINT-ENGINE-LIMITATIONS.md`.
- **Duplication pipeline impression** : structurelle (couche commune `blocs-tache.ts`). Plan de convergence documenté.
- **Observabilité prod** : plan timing + logs structurés + sémaphore Puppeteer documenté dans le même fichier.
